...
// Tags découpés en GROUPES sémantiques (dimensions). Chaque groupe est classé
// séparément → chaque dimension (couleur, technique, sujet, style…) sort SON
// meilleur candidat, on propose ensuite le top de chaque dimension. Vocabulaire
// VOLONTAIREMENT très large (demande 2026-07-19 « variété immense »), couvrant
// photo / illustration / design / motion-3D / mode / archi / œuvres de musée.
// `prompt` en anglais (SigLIP y est bien plus fiable). NB : les dimensions
// VISUELLES (couleur, teinte, sujet, matière, cadrage, technique) sont fiables ;
// les dimensions ABSTRAITES (ambiance, style, époque) sont indicatives — le
// modèle « devine » aux indices visuels, l'utilisateur décoche ce qui est faux.
